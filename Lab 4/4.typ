...
= 2. Experiment
The first part of the experimental setup consisted of a function generator and a series circuit consisting of an LED combined with a $100 Omega$ resistor. The various colors of LEDs included red, green, blue, and yellow.
...
The subsequent diffraction peak measurements for each color LED in pixels were multiplied by this value of $1.508$ to obtain the wavelength $lambda$ of its color. The calculated $lambda$ for each color LED were as followed:
...
Yellow = 593nm
`)
The inverse of each wavelength $1/lambda$ was calculated. These data pairs ($1/lambda$, $V_k$) were then plotted, with $V_k$ on the y-axis and $1/lambda$ on the x-axis, as shown in Figure 1 below.
...
The experimental value for $frac(h c, e)$ was determined to be $1367 plus.minus 351$ V nm. This value is in agreement with the accepted theoretical value of $1240$ V nm, as evidenced by a $t'$ score of $0.4$.
