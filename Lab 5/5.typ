...
#let abstract = [
  An electron diffraction experiment was performed to determine Planck's constant, $h$. Two measurement techniques, a manual tape-marking method and a digital image analysis method, were compared. The primary method using digital image analysis yielded values of $h_1 = (6.4 plus.minus 0.5) dot 10^(-34) " J s"$ and $h_2 = (6.6 plus.minus 0.6) dot 10^(-34) " J s"$. These results are in agreement with the accepted value, confirmed by t'-scores of $0.46$ and $0.04$. In contrast, the manual method produced inconclusive values of $h_1 = (5.3 plus.minus 0.6) dot 10^(-34) " J s"$ and $h_2 = (5.9 plus.minus 0.3) dot 10^(-34) " J s"$, with t'-scores of about 2. This comparison highlights the significant reduction in procedural uncertainty achieved through image analysis.
];
...
`)
As this electron beam passes through the carbon film, it diffracts. For a first-order diffraction peak ($n=1$) at a small angle $theta$, the condition for constructive interference is approximated by $lambda = 2d theta$, where $d$ is the spacing between atomic planes in the crystal lattice, a consequence of Bragg's law. The carbon film used has two primary lattice spacings, $d_1 = 0.213$ nm and $d_2 = 0.123$ nm, which produce two distinct diffraction rings. 
...
The initial data was collected using the manual method, where ring diameters were marked by hand on tape. Figure 1 displays the plot of $theta$ versus $V^(-1/2)$ for this data. The analysis yielded slopes of $2.3 plus.minus 0.3 (V)^"1/2"$ for the inner ring ($theta _1$), and $4.5 plus.minus 0.3 (V)^"1/2"$ for the outer ring ($theta _2$). These were then used to calculate the values for the hypothetical Planck's constant of $h_1 = (5.3 plus.minus 0.6) dot 10^(-34) " J s"$ and $h_2 = (5.9 plus.minus 0.3) dot 10^(-34) " J s"$ respectively. To evaluate these results against the accepted value, $h_"acc" = 6.626 dot 10^(-34) " J s"$, t'-scores were calculated. The scores were 2.15 for $h_1$ and 2.02 for $h_2$. Thse t'-scores greater than 1 but less than 3  indicates neither disagreement nor agreement between the data and the accepted value. Therefore, the results from the manual method are considered inconclusive, likely due to compounding parallax and measurement errors.
